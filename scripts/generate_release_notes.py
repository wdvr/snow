...
def get_current_version(repo_root: Path) -> str:
    ...
    # Try Info.plist
    for plist in (repo_root / "ios").glob("**/Info.plist"):
...
def generate_release_notes(app_type: str = "snow") -> str:
    ...
    ios_changes = [f for f in changed_files if f.startswith("ios/")]
...
def main():
    ...
    notes = generate_release_notes(app_type)
    ...
    repo_root = get_repo_root()
    output_path = (
        repo_root / "ios" / "fastlane" / "metadata" / "en-US" / "release_notes.txt"
    )
