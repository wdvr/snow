...
from ..models.resort import Resort, ElevationPoint, ElevationLevel
    ...
    def seed_initial_resorts(self) -> Dict[str, Any]:
        ...
        for resort_data in initial_resorts:
            try:
                ...
                if existing:
                    logger.info(f"Resort {resort_data.resort_id} already exists, skipping")
        ...
        logger.info(f"Resort seeding completed. Created: {results['resorts_created']}, "
    ...
    def get_resort_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for resort in resorts:
                ...
                elevations = [point.elevation_meters for point in resort.elevation_points]
            ...
            for resort in resorts:
                ...
                for point in resort.elevation_points:
                    if not (-90 <= point.latitude <= 90):
                        issues.append(f"Invalid latitude for {point.level.value}: {point.latitude}")
                    if not (-180 <= point.longitude <= 180):
                        issues.append(f"Invalid longitude for {point.level.value}: {point.longitude}")

                # Check elevation progression
                sorted_points = sorted(resort.elevation_points, key=lambda p: p.elevation_meters)
                for i in range(1, len(sorted_points)):
                    if sorted_points[i].elevation_meters <= sorted_points[i-1].elevation_meters:
